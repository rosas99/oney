...
ONEX_MODULE_NAME="github.com/rosas/onex"
...
OUTPUT_PKG="github.com/rosas/onex/pkg/generated"
...
function codegen::protobuf() {
    ...
    onex::util::read-array apis < <(
        git grep --untracked --null -l \
            ...
            | sed 's|^|github.com/rosas/onex/|;s|k8s.io/kubernetes/staging/src/||' \
            | sort -u)
...
function codegen::openapi() {
    ...
    local output_dir="pkg/generated/openapi"
    local output_pkg="github.com/rosas/onex/${output_dir}"
    local known_violations_file="${API_KNOWN_VIOLATIONS_DIR}/violation_exceptions.list"
    ...
    openapi-gen \
        ...
        --go-header-file "${BOILERPLATE_FILENAME}" \
        -O "${output_file}" \
        -i 'k8s.io/apimachinery/pkg/apis/meta/v1,k8s.io/apimachinery/pkg/runtime,k8s.io/apimachinery/pkg/version,k8s.io/apimachinery/pkg/util/intstr,k8s.io/kubernetes/pkg/apis/core,k8s.io/api/core/v1,k8s.io/api/autoscaling/v1,k8s.io/api/coordination/v1,k8s.io/kubernetes/pkg/apis/flowcontrol,k8s.io/api/flowcontrol/v1,k8s.io/apiextensions-apiserver/pkg/apis/apiextensions,k8s.io/apiextensions-apiserver/pkg/apis/apiextensions/v1,k8s.io/kube-aggregator/pkg/apis/apiregistration,k8s.io/kube-aggregator/pkg/apis/apiregistration/v1,github.com/rosas/onex/pkg/apis/apps/v1beta1' \
...
function codegen::listers() {
    ...
    onex::util::read-array ext_apis < <(
        cd "${ONEX_ROOT}"
        git_find -z ':(glob)pkg/apis/**/*types.go' \
            | while read -r -d $'\0' F; do dirname "github.com/rosas/onex/${F}"; done \
            | sort -u)
    ext_apis+=("${EXTRA_GENERATE_PKG[@]}")
